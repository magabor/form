    ...
    var createControls = function (that, header) {
        ...
        $.each(that.columns, function (i, column) {
            ...
            if (!column.filterControl) {
                ...
                var tag = that.$el[0].id + "_" + column.field;
                filterList.push(tag);
                switch (column.filterControl.toLowerCase()) {
                    case 'input' :                            
                        html.push(sprintf('<input type="text" id="' + tag + '" class="form-control" style="width: 100%; visibility: %s">', isVisible));
                        break;
                    case 'select':
                        html.push(sprintf('<select id="' + tag + '" class="%s form-control" style="width: 100%; visibility: %s"></select>',
            ...
            vR.reportTabFilters["#tab" + that.$el[0].id] = filterList;
        ...
        this.data = fp ? $.grep(this.data, function (item, i) {
            for (var key in fp) {
                ...
                if (value === "" || value === null || value === undefined)
                    return false;
/*
                if (thisColumn.searchFormatter===true)
    ...
    BootstrapTable.prototype.onColumnSearch = function (event) {
        ...
        // TeleworX: After searching using the inputs on each column, table got destroyed, so better to call resizeTable(delay=0) afterwards.
        vR.infoWindowManager.resizeTable(0);
    };
